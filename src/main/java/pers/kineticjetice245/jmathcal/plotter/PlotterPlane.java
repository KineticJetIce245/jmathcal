...
import pers.kineticjetice245.jmathcal.number.UndefinedValueException;
...
public class PlotterPlane {
    ...
    private void putGridLines(ArrayList<Node> array, double gridScale, double gridDiv, Color color, double lineWidth,
        ...
        double[] planeXSize = { origin[0], origin[0] + length[0] };
        double[] planeYSize = { -(origin[1] + length[1]), -origin[1] };

        // Grids
        while ((adj = count * actualScale) < length[i] + actualScale) {
            ...
            if (ifX) {
                if (curtLoc > planeXSize[0] && curtLoc < planeXSize[1])
                    array.add(gridLines);

            } else {
                if (curtLoc > planeYSize[0] && curtLoc < planeYSize[1])
                    array.add(gridLines);
            }
            count++;
            if ((gridDiv == 1) && ifNum) {
